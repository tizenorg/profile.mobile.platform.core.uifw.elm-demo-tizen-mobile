...
screen_reader_terminate(void)
{
	...
	if (res != APP_MANAGER_ERROR_NONE) {
		LOGE("Can't get screen reader app context - error code: %i", res);
		return;
	}
	LOGD("app_context: %p", app_context);
	res = app_manager_request_terminate_bg_app(app_context);

	if (res != APP_MANAGER_ERROR_NONE) {
		LOGE("Can't terminate screen reader app - error code: %i", res);
		return;
	}
}
