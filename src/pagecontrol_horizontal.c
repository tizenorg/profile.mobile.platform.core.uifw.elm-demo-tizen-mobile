...
_index_change_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	selected_page = (int)elm_object_item_data_get(it);
	...
	elm_atspi_accessible_translation_domain_set(pd->index, PACKAGE);
...
create_index(Evas_Object *parent)
{
	...
	elm_index_item_selected_set(it, EINA_TRUE);
	sprintf(buf, _("IDS_INDEX_PAGECONTROL_DESCRIPTION"), (int)elm_object_item_data_get(it) + 1, pd->total_page);
