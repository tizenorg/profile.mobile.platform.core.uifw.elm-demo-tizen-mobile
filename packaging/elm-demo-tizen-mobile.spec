...
%build
rm -rf CMakeFiles CMakeCache.txt && cmake . -DCMAKE_INSTALL_PREFIX=%{_pkg_dir} -DPKG_MANIFEST_PATH=%{TZ_SYS_RO_PACKAGES}
make %{?jobs:-j%jobs}
...
cp %{_builddir}/%{buildsubdir}/%{name}.xml %{buildroot}/%{TZ_SYS_RO_PACKAGES}/%{name}.xml
